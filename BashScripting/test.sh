...
if	[[ $choice == "dodge" ]]
	then
		...
		string="You duck and roll out of the way! Of what.... I do not know."
		for ((i=0; i<=${#string}; i++)); do
    		printf '%s' "${string:$i:1}"
    		sleep 0.$(( RANDOM % 2 ))
		done  
		...
		then 
			...
			then
				...
				then
                    ...
                    if [[ $wolfhp -gt 0 ]]
                    then    
                        string="The wolf is still standing, It is in too much pain to attack!! What will you do? (attack or dodge)"
					    for ((i=0; i<=${#string}; i++)); do
    					    printf '%s' "${string:$i:1}"
   				 		    sleep 0.$(( RANDOM % 2 ))
					    done 
                        ...
                        then
                            string="You duck and roll out of the way! Of what.... I do not know."
		                            ...
		                            echo
                                    string="The wolf is still standing, It is priming an attack!! What will you do? (attack or dodge)"
					                for ((i=0; i<=${#string}; i++)); do
    					                printf '%s' "${string:$i:1}"
   				 		                sleep 0.$(( RANDOM % 2 ))
					                done 
...
then
            ...
            echo
            string="Shocked by your agility, the wolf takes a moment to consider it's move."
			for ((i=0; i<=${#string}; i++)); do
    			printf '%s' "${string:$i:1}"
   				sleep 0.$(( RANDOM % 2 ))
            done
            echo
            string="What will you do? (you can type: attack or dodge"
			for ((i=0; i<=${#string}; i++)); do
    			printf '%s' "${string:$i:1}"
   				sleep 0.$(( RANDOM % 2 ))
            done
...
if [[ $hp -lt 1 ]]
    then  
            string="YOU DIED!"
		        ...
		        echo
                string="On its dying throes, the wolf desperately slashes at you with its claws , you have $hp hit points remaining!!"
					for ((i=0; i<=${#string}; i++)); do
    					printf '%s' "${string:$i:1}"
   				 		sleep 0.$(( RANDOM % 2 ))
					done 
                    echo
                    string="You end the battle battered and bruised, but you find that it was worth it. You will forever be know as ${bold}WOLF'S BANE${normal}. You return home as a hero!!"
					for ((i=0; i<=${#string}; i++)); do
    					printf '%s' "${string:$i:1}"
   				 		sleep 0.$(( RANDOM % 2 ))
                    done
